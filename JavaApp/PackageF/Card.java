...
public class Card implements iCard {
    ...
    public int[] getAttacks(int i) {
        ...
        } else if (this.checkSigil(Sigils.TrifurcatedStrike)) {
            return attackLists[2];
        } else if (this.checkSigil(Sigils.OmniStrike)) {
    ...
    public int takeDamage(int damage, Card card) {
        ...
        if (damage > 0) {
            ...
            if (checkSigil(Sigils.SharpQuills)) {
                if (!(card == null)) {
                    card.takeDamage(1, null);// realistically, if i wanted this to be robust, I would always pass the
                                             // dmg src, and add a list of damage tags(poison, quill, etc). oh well
                }
            }
        }
        if (card == null) {
