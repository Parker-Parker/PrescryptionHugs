...
public class Field {
    ...
    public Boolean playCard(Card card, int slot) { // return true if successful, expect prev container to remove from
                                                   // its list
    ...
    String blank = "          ";
    String horiz = "+" + blank.replace(' ', '-') + "+" + blank.replace(' ', '-') + "+" + blank.replace(' ', '-') + "+"
            + blank.replace(' ', '-') + "+";
    ...
    public void enemyAttack(int i) {
        if (enemyCards[i] != null) {
            ...
            int[] targets = enemyCards[i].getAttacks(i);// (get pos based debuff/buff too)asks each card(slot) who it
                                                        // wants to attack, then does the attack(flight/jump/nope
                                                        // sigils)//must call attackee's takedamage function(handle
                                                        // death,overflow dmg, reflect)
            for (int t : targets) {
                if (((i + t) >= 0) && ((i + t) <= 3)) {// only attack if the attacker is looking for a valid target slot
                    // //get buffs
                    // int left = playerCards[i-1]== null ? 0 : (i>1 ? playerCards[i-1].getBuff(-1)
                    // : 0); // come back and add null handling
                    // int up = enemyCards[i] == null ? 0 : (true ? enemyCards[i].getBuff(0) : 0);
                    // // come back and add null handling
                    // int right = playerCards[i+1]== null ? 0 : (i<3 ? playerCards[i+1].getBuff(1)
                    // : 0); // come back and add null handlinge

                    // get buffs
                    int left = i > 0 ? (enemyCards[i - 1] == null ? 0 : enemyCards[i - 1].getBuff(-1)) : 0; // come back
                                                                                                            // and add
                                                                                                            // null
                                                                                                            // handling
                    int up = true ? (playerCards[i] == null ? 0 : playerCards[i].getBuff(0)) : 0; // come back and add
                                                                                                  // null handling
                    int right = i < 3 ? (enemyCards[i + 1] == null ? 0 : enemyCards[i + 1].getBuff(1)) : 0; // come back
                                                                                                            // and add
                                                                                                            // null
                                                                                                            // handlinge
                    ...
                    Card target;
                    if (playerCards[i + t] == null) {
                        target = this.nullCard;
                    } else {
                        target = playerCards[i + t];
                    } // handles null target, code below handles valid case...need to add handling for
                      // i+t oob
                    // check fly/block sigil if they match attack target, if they miss attack
                    // nullcard
                    // this.checkHit(atkCark,tgtCard);

                    // sigils: atk~ airborne def~ leap waterborne other def~ tail burrower //Tail
                    // can be done on tailCard.takeDamage(), burrower should be done every time an
                    // attack is "announced",(ask everycard on the field left to right if they want
                    // to burrow, if so attempt move to target slot then execute attack), both will
                    // require a "bool move(int slot)" method that tries to move the card to the
                    // given slot and returns true if successful. tail will need this bool verify if
                    // it takes damage or child card takes damage.
                    // x x x 1 miss
                    // 0 x x 0 hit
                    // 1 x 0 x miss
                    // 1 x 1 0 hit
                    // 0 x 0 0
                    // sigils: atk~ airborne def~repulsive leap waterborne // repulsive(prevent
                    // attack) should be checked on takeDamage(int,card), i think; only actually
                    // takes damage if sourcecard is null, since that would be revenge damagefrom
                    // spikes
                    ...
                    int overflow = target.takeDamage(damage, enemyCards[i]);// the damage source is passed in so the
                                                                            // spike sigil can be tested in takeDamage
                                                                            // and directly call the revenge target's
                                                                            // .takeDamage(1,null);
    ...
    public void playerAttack(int i) {
        if (playerCards[i] != null) {
            ...
            int[] targets = playerCards[i].getAttacks(i);// (get pos based debuff/buff too)asks each card(slot) who it
                                                         // wants to attack, then does the attack(flight/jump/nope
                                                         // sigils)//must call attackee's takedamage function(handle
                                                         // death,overflow dmg, reflect)
            for (int t : targets) {
                if (((i + t) >= 0) && ((i + t) <= 3)) {// only attack if the attacker is looking for a valid target slot
                    ...
                    // get buffs
                    int left = i > 0 ? (playerCards[i - 1] == null ? 0 : playerCards[i - 1].getBuff(-1)) : 0; // come
                                                                                                              // back
                                                                                                              // and add
                                                                                                              // null
                                                                                                              // handling
                    int up = true ? (enemyCards[i] == null ? 0 : enemyCards[i].getBuff(0)) : 0; // come back and add
                                                                                                // null handling
                    int right = i < 3 ? (playerCards[i + 1] == null ? 0 : playerCards[i + 1].getBuff(1)) : 0; // come
                                                                                                              // back
                                                                                                              // and add
                                                                                                              // null
                                                                                                              // handlinge
                    ...
                    // can actually move i+t(valid target slot 0-3) up here or earlier
                    Card target;
                    if (enemyCards[i + t] == null) {
                        target = this.nullCard;
                    } else {
                        target = enemyCards[i + t];
                    } // handles null target, code below handles valid case...need to add handling for
                      // i+t oob
                    // check fly/block sigil if they match attack target, if they miss attack
                    // nullcard
                    ...
                    // this is only for the main case, on hit
                    int overflow = target.takeDamage(damage, playerCards[i]);// the damage source is passed in so the
                                                                             // spike sigil can be tested in takeDamage
                                                                             // and directly call the revenge target's
                                                                             // .takeDamage(1,null);
    ...
    public void purgeDead() {
        for (int i = 0; i < 4; i++) {
            this.enemyCardsBack[i] = (this.enemyCardsBack[i] == null) ? null
                    : (this.enemyCardsBack[i].checkDead() ? null : this.enemyCardsBack[i]);
            this.enemyCards[i] = (this.enemyCards[i] == null) ? null
                    : (this.enemyCards[i].checkDead() ? null : this.enemyCards[i]);
            this.playerCards[i] = (this.playerCards[i] == null) ? null
                    : (this.playerCards[i].checkDead() ? null : this.playerCards[i]);
        }
    }

    public void purge(Card card) {
        for (int i = 0; i < 4; i++) {
            if (this.enemyCardsBack[i] == card) {
                this.enemyCardsBack[i] = null;
            }
            if (this.enemyCards[i] == card) {
                this.enemyCards[i] = null;
            }
