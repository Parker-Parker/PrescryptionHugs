...
public class TurnController {
    ...
    // make a "move" object called nextMove that holds what leshy will play to the
    // back next
    // make a "script/itinerary" object that holds or fetches multiple "moves", and
    ...
    public void executeState() {
        ...
        switch (this.state) {
            ...
            case playerReady:
                ...
                // this.setState(TurnState.playerSacrifice); //if player attempts summon of 1+
                // cost
            ...
            case playerSacrifice:
                // collect respond to player input
                this.setState(TurnState.playerSummon); // if summon cost satisfied //call sacrifice method on all cards
                                                       // in list
            ...
            case playerSummon:
                ...
                // this.setState(TurnState.playerReady); //if summon unsuccessful and there was
                // no sacrifice
                // this.setState(TurnState.playerSummon);//if summon unsuccessful but sacrifice
                // succeeded
                break;
            case playerEnd:
            ...
            case playerAttack:
                for (int i = 0; i < 4; i++) {
                    field.playerAttack(i);// (get pos based debuff/buff too)asks each card(slot) who it wants to attack,
                                          // then does the attack(flight/jump/nope sigils)//must call attackee's
                                          // takedamage function(handle death,overflow dmg, reflect)
                }
            ...
            case leshyAttack:
                for (int i = 0; i < 4; i++) {
                    field.enemyAttack(i); // (get pos based debuff/buff too)asks each card(slot) who it wants to attack,
                                          // then does the attack(flight/jump/nope sigils)//must call attackee's
                                          // takedamage function(handle death,overflow dmg, reflect)
                }
            ...
            case leshyEnd:
                // leshyPlanNextMove()
                // leshyfetchnextmove() //fetches next move from a move itinerary that can be
                // edited asynchronously
                for (int i = 0; i < 4; i++) {
